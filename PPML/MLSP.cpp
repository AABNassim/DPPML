...
void MLSP::test_key_exchange() {
    ...
    if (send_long(sock, 42)) {
        ...
        send_file(sock, "test_ciphertext.txt");


        // Multiplication
        read_file(sock, "serkey/MULTIPLICATION_KEY.txt");
        ...
        if (send_file(sock, "cipher_mult.txt")) {
        ...
        // send the cipher
        if (send_file(sock, "cipher_rot.txt")) {
        ...
        // send the cipher
        if (send_file(sock, "cipher_right_rot.txt")) {
